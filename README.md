...
## 实测结果 D · 档案馆形态的批次（2026-09-07 新增）
校内档案部门 09-07 回信描述的形态：按个案扫描、600 dpi TIFF 保存件 + 300 dpi JPEG 访问件、历年做法不一致。据此改了三处：
- **`--access-dpi N`**：JPEG 按访问件处理，与母片按文件名主干配对（`photo_0007.tif` ↔ `photo_0007.jpg`）。清晰度、重复、序号、色卡区比对都只在同角色内做——访问件天然是母片的近似重复、也天然更软，不能拿去和母片比。缺配对的一方各报 `pairing` 警告。VLM 只看母片。不给这个参数时行为与之前逐字节相同（JPEG 仍判格式失败）。
- **分辨率元数据「未设置」与「值错了」分开**：无标签的 TIFF 在 PIL 里读出来是 (1, 1)，无 JFIF 密度的 JPEG 读出来是空，二者都归 `dpi` 警告 → review，不再是 reprocess。写错的值（如 150 对 600）仍是 fail → reprocess。
- **16 位 TIFF 按量程缩到 8 位再测量**：此前 `convert("L")` 把它截成一片白，曝光与反光双双误报。

合成夹具 `python3 cli.py fixtures runs/x --kind archive`（5 页母片 + 访问件，注入：无元数据、16 位、缺访问件、孤儿访问件），`qc --dpi 600 --access-dpi 300`：**accept 7 / review 3，零 reshoot、零误报**；旧两批结果不变。六个新测试先在旧代码上跑红（无角色、访问件被判为母片的重复、缺元数据判 fail、16 位读成全白、无配对）再实现。

**证据层级不变：仍全是合成。** 真实文件夹见 #8。
